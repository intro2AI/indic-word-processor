...
class TextEdit_Hindi(QTextEdit):
    ...
    def keyPressEvent(self, event):
        ...
        if event.key() in [QtCore.Qt.Key_Enter,QtCore.Qt.Key_Return]:
            ...
            return super(TextEdit_Hindi, self).keyPressEvent(event) 
    ...
    def keyReleaseEvent(self, event):
        ...
        if event.key() == QtCore.Qt.Key_QuoteLeft:  # Backtick key
            self.is_backtick_pressed = False
        return super(TextEdit_Hindi, self).keyReleaseEvent(event)
    ...
    def canInsertFromMimeData(self, source):
        ...
        if source.hasImage():
            ...
            return super(TextEdit_Hindi, self).canInsertFromMimeData(source)
    ...
    def insertFromMimeData(self, source):
        ...
        super(TextEdit_Hindi, self).insertFromMimeData(source)
...
class TextEdit_Marathi(QTextEdit):
    def __init__(self,label_trackAllKeys, *args, **kwargs):
        super(self.__class__, self).__init__(*args, **kwargs)
    ...
    def keyPressEvent(self, event):
        ...
        if event.key() in [QtCore.Qt.Key_Enter,QtCore.Qt.Key_Return]:
            ...
            self.label_trackAllKeys.setText(self.label_trackAllKeys.text()+'\n')
            #self.label_trackScriptKeys.setText(self.label_trackScriptKeys.text()+"\n")
            return super(TextEdit_Marathi, self).keyPressEvent(event) 
        elif event.key() in [QtCore.Qt.Key_Left,QtCore.Qt.Key_Right]: #, QtCore.Qt.AltModifier
            return super(TextEdit_Marathi, self).keyPressEvent(event) 
        elif event.key() in [QtCore.Qt.Key_Shift]:
            return super(TextEdit_Marathi, self).keyPressEvent(event)
    ...
    def keyReleaseEvent(self, event):
        ...
        if event.key() == QtCore.Qt.Key_QuoteLeft:  # Backtick key
            self.is_backtick_pressed = False
        return super(TextEdit_Marathi, self).keyReleaseEvent(event)
    ...
    def canInsertFromMimeData(self, source):
        ...
        if source.hasImage():
            ...
            return super(TextEdit_Marathi, self).canInsertFromMimeData(source)
    ...
    def insertFromMimeData(self, source):
        ...
        super(TextEdit_Marathi, self).insertFromMimeData(source)
...
class TextEdit_Prakrit(QTextEdit):
    def __init__(self,label_trackAllKeys, *args, **kwargs):
        super(self.__class__, self).__init__(*args, **kwargs)
        ...
        self.current_script: str = ""
        # IMPORTING HINDI MAPPING DICTIONARIES
        from mappings.mappings_prakrit import C,V,v,misc
        self.C = C #consonents
        self.V = V #Independent Vowels
        self.v = v #dependent vowels
        self.misc = misc #nukta, halant, numbers and misc symbols
    ...
    def insertPlainText(self, text):
        """
        Override insertPlainText to check if text is allowed before insertion
        
        Args:
            text (str): Text to be inserted
        """
        if text in self.allowed_text:
            # Get current cursor
            cursor = self.textCursor()
    ...
    def keyPressEvent(self, event):
        ...
        if event.key() in [QtCore.Qt.Key_Enter,QtCore.Qt.Key_Return]:
            ...
            self.label_trackAllKeys.setText(self.label_trackAllKeys.text()+'\n')
            #self.label_trackScriptKeys.setText(self.label_trackScriptKeys.text()+"\n")
            return super(TextEdit_Prakrit, self).keyPressEvent(event) 
        elif event.key() in [QtCore.Qt.Key_Left,QtCore.Qt.Key_Right]: #, QtCore.Qt.AltModifier
            return super(TextEdit_Prakrit, self).keyPressEvent(event) 
        elif event.key() in [QtCore.Qt.Key_Shift]:
            return super(TextEdit_Prakrit, self).keyPressEvent(event)
        ...
        if event.modifiers() & QtCore.Qt.ControlModifier:
            ...
            #Dependent when last is consonent, or last is in a,e,i,o,u
                ...
                if key_2 in ['Zg','Zj','ZD','Zb','ZK']: # no auto-halant after sindhi letters, and other rare consonents
                    ...
                    self.deletePreviousChars(1,cursor,self.C[key_2])
                    self.insertPlainText(self.C[key_2])
                    # AUTO ADD HALANT (if key_2[0] not 'q')
                    self.insertPlainText(self.misc['q'])
                    self.label_trackAllKeys.setText(self.label_trackAllKeys.text()+'q')
                ...
                print("in key_2")
                self.deletePreviousChars(1,cursor,self.misc[key_2])
                self.insertPlainText(self.misc[key_2])
                self.makeNextVowelDependent = False
                self.use2CharsVowelNext = True # every misc element resets this FLAG aai, taai vs m'ou'
                ...
                if key_2 == 'q ':
                    #if a space ' ' follows an ununsed halant, first delete the halant, and then add the space.
                    self.deletePreviousChars(1,cursor,self.misc[' '])
                    self.insertPlainText(self.misc[' '])
                    self.makeNextVowelDependent = False
                    self.use2CharsVowelNext = True # every consonent resets this FLAG aai, taai vs m'ou'
                elif key_1 in ['w','W']: #Inserting ZWNJ or ZWJ
            ...
            # for a given language, we want to check if key 3 is in repective english mappings of ृ, ऋ, ॠ
                ...
                print("in key_2")
                self.deletePreviousChars(1,cursor,self.V[key_2])
                self.insertPlainText(self.v[key_2])
                self.makeNextVowelDependent = False
            ...
            # CONTEXT BASED LOGIC
                ...
                if key_2 == 'qa':
                    ...
                    print('auto deleting halant..')
                    self.deletePreviousChars(1,cursor,self.v['i'])
                    self.insertPlainText(self.v['i'])
                    self.makeNextVowelDependent = True
                elif key_2 == 'qo':
                    print('auto deleting halant..')
                    self.deletePreviousChars(1,cursor,self.v['o'])
                    self.insertPlainText(self.v['o'])
                    self.makeNextVowelDependent = True
                elif key_2 == 'qe':
                    print('auto deleting halant..')
                    self.deletePreviousChars(1,cursor,self.v['e'])
                    self.insertPlainText(self.v['e'])
                    self.makeNextVowelDependent = True
                elif key_2 == 'qu':
                    print('auto deleting halant..')
                    self.deletePreviousChars(1,cursor,self.v['u'])
                    self.insertPlainText(self.v['u'])
                    self.makeNextVowelDependent = True
                elif key_2 == 'q ':
                    print('auto deleting halant..')
    ...
    def keyReleaseEvent(self, event):
        ...
        if event.key() == QtCore.Qt.Key_QuoteLeft:  # Backtick key
            self.is_backtick_pressed = False
        return super(TextEdit_Prakrit, self).keyReleaseEvent(event)
                                                                                                                                                                                                                                                                                                                         
    # def onCursorPositionChangedByMouseOrArrow(self,pos):
    #   CLEAR label_trackAllKeys TEXT

    def deletePreviousChars(self, n, cursor, char=None):
        """
        Delete the previous n characters while maintaining text formatting.
        If there are fewer than n chars before the cursor, delete what's available.
        """
        if char in self.allowed_text:
            # Store the current position
            current_position = cursor.position()
            # Calculate the start position for deletion
            start_position = max(1, current_position - n)
    ...
    def canInsertFromMimeData(self, source):
        ...
        if source.hasImage():
            ...
            return super(TextEdit_Prakrit, self).canInsertFromMimeData(source)
    ...
    def insertFromMimeData(self, source):
        ...
        super(TextEdit_Prakrit, self).insertFromMimeData(source)
...
class TextEdit_Indus(QTextEdit):
    def __init__(self,label_trackAllKeys, *args, **kwargs):
        ...
        self.current_script: str = ""
        # IMPORTING HINDI MAPPING DICTIONARIES
        from mappings.mappings_indus import C,V,v,misc   #TODO change these to be indus by default
        self.C = C #consonents
        self.V = V #Independent Vowels
        self.v = v #dependent vowels
        self.misc = misc #nukta, halant, numbers and misc symbols
    ...
    def keyPressEvent(self, event):
        ...
        if event.key() in [QtCore.Qt.Key_Enter,QtCore.Qt.Key_Return]:
            ...
            self.label_trackAllKeys.setText(self.label_trackAllKeys.text()+'\n')
            #self.label_trackScriptKeys.setText(self.label_trackScriptKeys.text()+"\n")
            return super(TextEdit_Indus, self).keyPressEvent(event) 
        elif event.key() in [QtCore.Qt.Key_Left,QtCore.Qt.Key_Right]: #, QtCore.Qt.AltModifier
            return super(TextEdit_Indus, self).keyPressEvent(event) 
        elif event.key() in [QtCore.Qt.Key_Shift]:
            return super(TextEdit_Indus, self).keyPressEvent(event)
    ...
    def canInsertFromMimeData(self, source):
        ...
        if source.hasImage():
            ...
            return super(TextEdit_Indus, self).canInsertFromMimeData(source)
    ...
    def insertFromMimeData(self, source):
        ...
        super(TextEdit_Indus, self).insertFromMimeData(source)
...
class TextEdit_Sanskrit(QTextEdit):
    def __init__(self,label_trackAllKeys, *args, **kwargs):
        super(self.__class__, self).__init__(*args, **kwargs)
        ...
        self.current_script: str = ""
        # IMPORTING HINDI MAPPING DICTIONARIES
        from mappings.mappings_sanskrit import C,V,v,misc
        self.C = C #consonents
        self.V = V #Independent Vowels
        self.v = v #dependent vowels
        self.misc = misc #nukta, halant, numbers and misc symbols
    ...
    def keyPressEvent(self, event):
        ...
        if event.key() in [QtCore.Qt.Key_Enter,QtCore.Qt.Key_Return]:
            ...
            self.label_trackAllKeys.setText(self.label_trackAllKeys.text()+'\n')
            #self.label_trackScriptKeys.setText(self.label_trackScriptKeys.text()+"\n")
            return super(TextEdit_Sanskrit, self).keyPressEvent(event) 
        elif event.key() in [QtCore.Qt.Key_Left,QtCore.Qt.Key_Right]: #, QtCore.Qt.AltModifier
            return super(TextEdit_Sanskrit, self).keyPressEvent(event) 
        elif event.key() in [QtCore.Qt.Key_Shift]:
            return super(TextEdit_Sanskrit, self).keyPressEvent(event)
    ...
    def keyReleaseEvent(self, event):
        ...
        if event.key() == QtCore.Qt.Key_QuoteLeft:  # Backtick key
            self.is_backtick_pressed = False
        return super(TextEdit_Sanskrit, self).keyReleaseEvent(event)
    ...
    def canInsertFromMimeData(self, source):
        ...
        if source.hasImage():
            ...
            return super(TextEdit_Sanskrit, self).canInsertFromMimeData(source)
    ...
    def insertFromMimeData(self, source):
        ...
        super(TextEdit_Sanskrit, self).insertFromMimeData(source)
